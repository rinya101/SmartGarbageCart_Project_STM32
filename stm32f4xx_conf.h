...
#include "misc.h" /* High level functions for NVIC and SysTick (add-on to CMSIS functions) */
                  /* 用于NVIC和SysTick的高级函数（CMSIS函数的附加功能）*/
...
/* 如果使用外部时钟源，则应将以下宏定义的值设置为外部时钟源的值；否则，如果不使用外部时钟，保持此宏定义处于注释状态 */
/*#define I2S_EXTERNAL_CLOCK_VAL   12288000 */ /* 外部时钟的频率值，单位为Hz */


/* 取消下面一行的注释，以在标准外设库驱动代码中展开"assert_param"宏 */
/* #define USE_FULL_ASSERT    1 */
/* Exported macro ------------------------------------------------------------*/
...
/**
  ...
  * @param  expr：如果expr为假，它会调用assert_failed函数，
  *   该函数会报告调用失败的源文件名和源文件行号。
  *   如果expr为真，则不返回任何值。
  * @retval 无
  */
