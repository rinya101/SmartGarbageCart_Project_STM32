 ...
 * @param cfg 
 */
static void usart_clock_init(message_handle_t* handle, message_cfg_t* cfg)
{
    RCC_APB2PeriphClockCmd(cfg->usart_rcc, ENABLE);
    RCC_AHB1PeriphClockCmd(cfg->tx_pin_rcc, ENABLE);
    RCC_AHB1PeriphClockCmd(cfg->rx_pin_rcc, ENABLE);
}
/**
 * @brief 串口 GPIO 初始化
 * 
 ...
 * @param cfg 
 */
static void usart_gpio_init(message_handle_t* handle, message_cfg_t* cfg)
{
   /* GPIO 复用配置 */
    ...
    GPIO_InitStructure.GPIO_Mode    = cfg->tx_mode;
    GPIO_InitStructure.GPIO_OType   = cfg->tx_otype;
    GPIO_InitStructure.GPIO_PuPd    = cfg->tx_pu;
    GPIO_InitStructure.GPIO_Speed   = cfg->tx_speed;
    GPIO_Init(cfg->tx_port, &GPIO_InitStructure);
    /* RX GPIO 初始化 */
    GPIO_InitStructure.GPIO_Pin     = cfg->rx_pin;
    GPIO_InitStructure.GPIO_Mode    = cfg->rx_mode;
    GPIO_InitStructure.GPIO_OType   = cfg->rx_otype;
    GPIO_InitStructure.GPIO_PuPd    = cfg->rx_pu;
    GPIO_InitStructure.GPIO_Speed   = cfg->rx_speed;
 ...
 * @param cfg 
 */
static void usart_init(message_handle_t* handle, message_cfg_t* cfg)
{
 ...
 * @param cfg 
 */
static void usart_use_dma_init(message_handle_t* handle, message_cfg_t* cfg)
{
 ...
 * @param cfg 
 */
static void usart_irq_init(message_handle_t* handle, message_cfg_t* cfg)
{
...
void bsp_message_send_string(message_handle_t* handle, char* str, uint16_t len)
{
    while (*str != '\0' && len--)
    {
        bsp_message_send_byte(handle, *str);
        str++;
    }
}
...
{
    bsp_message_send_byte(s_message, (uint8_t)ch);
    return ch;
}