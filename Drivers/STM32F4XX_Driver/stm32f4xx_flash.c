...
/**
  ...
  * @note 对于STM32F405xx/407xx、STM32F415xx/417xx、STM32F401xx/411xE/STM32F412xG以及STM32F413_423xx器件
  *       此参数的值可以在FLASH_Latency_0到FLASH_Latency_7之间。
  *
  * @note For STM32F42xxx/43xxx devices this parameter can be a value between 
  *       FLASH_Latency_0 and FLASH_Latency_15. 
  * @note 对于STM32F42xxx/43xxx器件，此参数的值可以在
  *       FLASH_Latency_0到FLASH_Latency_15之间。
  *         
  * @retval None
  */
void FLASH_SetLatency(uint32_t FLASH_Latency)
...
/**
  * @brief  Erases a specified FLASH Sector.擦除指定的FLASH扇区。
  *
...
/**
  ...
  */
/**
  * @brief  在指定地址编程一个字节（8位）。
  * @note   此函数可在所有设备电源电压范围内使用。
  *
  * @note   如果同时请求擦除和编程操作，
  *         则擦除操作会在编程操作之前执行。
  *
  * @param  Address: 指定要编程的地址。
  *         该参数可以是程序存储区或OTP区中的任何地址。
  * @param  Data: 指定要编程的数据。
  * @retval FLASH状态：返回值可以是：FLASH_BUSY、FLASH_ERROR_PROGRAM、
  *                     FLASH_ERROR_WRP、FLASH_ERROR_OPERATION或FLASH_COMPLETE。
  */
FLASH_Status FLASH_ProgramByte(uint32_t Address, uint8_t Data)
{
  FLASH_Status status = FLASH_COMPLETE;
