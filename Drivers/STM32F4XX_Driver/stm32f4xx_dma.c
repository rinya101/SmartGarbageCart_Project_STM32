...
@verbatim   
 ...
 ===============================================================================  
    ...
    本小节提供的函数可用于初始化DMA流的源地址和目的地址、增量及数据大小、传输方向、缓冲区大小、
    循环/正常模式选择、
    存储器到存储器模式选择以及流优先级值。
    [..]
    ...
    DMA_Init()函数遵循参考手册（RM0090）中所述的DMA配置步骤，
    但有一点除外：等待EN位复位。用户应用程序在调用DMA_Init()函数之前，
    应使用DMA_GetCmdStatus()函数检查此条件。
...
/**
  ...
  */
/**
  * @brief  将DMAy Streamx寄存器重新初始化为其默认重置值。
...
/**
  ...
  */
/**
  * @brief  根据DMA_InitStruct结构体中指定的参数初始化DMAy Streamx。
  * @note   调用此函数之前，建议使用DMA_GetCmdStatus()函数检查该流是否确实已禁用。
  * @param  DMAy_Streamx：其中y可以是1或2，用于选择DMA；x可以是0到7，用于选择DMA流。
  * @param  DMA_InitStruct：指向DMA_InitTypeDef结构体的指针，该结构体包含指定DMA流的配置信息。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  为每个DMA_InitStruct成员填充其默认值。
  * @param  DMA_InitStruct：指向将要被初始化的DMA_InitTypeDef结构体的指针。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  当PINC（外设地址递增模式）位被置位时，配置外设地址应按数据大小（由PSIZE位配置）递增，还是按固定偏移量4（32位对齐地址）递增。
  *   
  * @note   如果外设递增模式被禁用，此函数无效。
  *     
  * @param  DMAy_Streamx：其中y可以是1或2，用于选择DMA；x可以是0到7，用于选择DMA流。
  * @param  DMA_Pincos：指定外设递增偏移量大小。
  *          此参数可以是以下值之一：
  *            @arg DMA_PINCOS_Psize：外设地址递增根据PSIZE参数进行。
  *            @arg DMA_PINCOS_WordAligned：外设地址递增偏移量固定为4（32位对齐地址）。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  向DMAy Streamx写入要传输的数据单元数量。
  * @param  DMAy_Streamx：其中y可以是1或2，用于选择DMA；x可以是0到7，用于选择DMA流。
  * @param  Counter：要传输的数据单元数量（范围为0到65535）。数据项的数量仅取决于外设的数据格式。
  *            
  * @note   如果外设的数据格式是字节（Bytes）：数据单元数量等于要传输的总字节数。
  *           
  * @note   如果外设的数据格式是半字（Half-Word）：数据单元数量等于要传输的总字节数除以2。
  *           
  * @note   如果外设的数据格式是字（Word）：数据单元数量等于要传输的总字节数除以4。
  *      
  * @note   在存储器到存储器（Memory-to-Memory）传输模式下，DMAy_SxPAR寄存器所指向的存储缓冲区被视为外设。
  *      
...
/**
  ...
  */
/**
  * @brief  检查指定的DMAy Streamx标志位是否已设置。
  * @param  DMAy_Streamx：其中y可以是1或2，用于选择DMA；x可以是0到7，用于选择DMA流。
  * @param  DMA_FLAG：指定要检查的标志位。
  *          此参数可以是以下值之一：
  *            @arg DMA_FLAG_TCIFx：流x传输完成标志
  *            @arg DMA_FLAG_HTIFx：流x半传输完成标志
  *            @arg DMA_FLAG_TEIFx：流x传输错误标志
  *            @arg DMA_FLAG_DMEIFx：流x直接模式错误标志
  *            @arg DMA_FLAG_FEIFx：流x FIFO错误标志
  *         其中x可以是0到7，用于选择DMA流。
  * @retval DMA_FLAG的新状态（SET或RESET）。
  */
FlagStatus DMA_GetFlagStatus(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_FLAG)
{
...
/**
  ...
  */
/**
  * @brief  检查指定的DMAy Streamx中断是否已发生。
  * @param  DMAy_Streamx：其中y可以是1或2，用于选择DMA；x可以是0到7，用于选择DMA流。
  * @param  DMA_IT：指定要检查的DMA中断源。
  *          此参数可以是以下值之一：
  *            @arg DMA_IT_TCIFx：流x传输完成中断
  *            @arg DMA_IT_HTIFx：流x半传输完成中断
  *            @arg DMA_IT_TEIFx：流x传输错误中断
  *            @arg DMA_IT_DMEIFx：流x直接模式错误中断
  *            @arg DMA_IT_FEIFx：流x FIFO错误中断
  *         其中x可以是0到7，用于选择DMA流。
  * @retval DMA_IT的新状态（SET或RESET）。
  */
ITStatus DMA_GetITStatus(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t DMA_IT)
{
