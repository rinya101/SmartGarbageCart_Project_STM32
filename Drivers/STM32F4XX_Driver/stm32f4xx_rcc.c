...
/**
  * @brief  配置外部高速振荡器（HSE）。
  * @note   使能HSE（RCC_HSE_ON或RCC_HSE_Bypass）后，应用程序软件应等待HSERDY标志置位，这表明HSE时钟已稳定，可用于为PLL和/或系统时钟提供时钟。
  * @note   如果HSE直接用作系统时钟或通过PLL用作系统时钟，则其状态无法更改。在这种情况下，必须选择另一个系统时钟源，然后再更改HSE的状态（例如，禁用它）。
  * @note   进入停机模式（STOP）和待机模式（STANDBY）时，HSE会被硬件停止。
  * @note   此函数会复位CSSON位，因此如果之前已启用时钟安全系统（CSS），则必须在调用此函数后重新启用它。
  * @param  RCC_HSE：指定HSE的新状态。
  *          此参数可以是以下值之一：
  *            @arg RCC_HSE_OFF：关闭HSE振荡器，HSERDY标志在6个HSE振荡器时钟周期后变低。
  *            @arg RCC_HSE_ON：开启HSE振荡器
  *            @arg RCC_HSE_Bypass：HSE振荡器由外部时钟旁路
  * @retval 无
  */
...
/**
  * @brief  使能或禁用AHB1外设时钟。
  * @note   复位后，外设时钟（用于寄存器读写访问）处于禁用状态，应用程序软件在使用该时钟前必须将其使能。
  * @param  RCC_AHBPeriph: 指定要控制时钟的AHB1外设。
  *          该参数可以是以下值的任意组合：
  *            @arg RCC_AHB1Periph_GPIOA:       GPIOA时钟
  *            @arg RCC_AHB1Periph_GPIOB:       GPIOB时钟
  *            @arg RCC_AHB1Periph_GPIOC:       GPIOC时钟
  *            @arg RCC_AHB1Periph_GPIOD:       GPIOD时钟
  *            @arg RCC_AHB1Periph_GPIOE:       GPIOE时钟
  *            @arg RCC_AHB1Periph_GPIOF:       GPIOF时钟
  *            @arg RCC_AHB1Periph_GPIOG:       GPIOG时钟
  *            @arg RCC_AHB1Periph_GPIOG:       GPIOG时钟
  *            @arg RCC_AHB1Periph_GPIOI:       GPIOI时钟
  *            @arg RCC_AHB1Periph_GPIOJ:       GPIOJ时钟（STM32F42xxx/43xxx器件）
  *            @arg RCC_AHB1Periph_GPIOK:       GPIOK时钟（STM32F42xxx/43xxx器件）
  *            @arg RCC_AHB1Periph_CRC:         CRC时钟
  *            @arg RCC_AHB1Periph_BKPSRAM:     BKPSRAM接口时钟
  *            @arg RCC_AHB1Periph_CCMDATARAMEN CCM数据RAM接口时钟
  *            @arg RCC_AHB1Periph_DMA1:        DMA1时钟
  *            @arg RCC_AHB1Periph_DMA2:        DMA2时钟
  *            @arg RCC_AHB1Periph_DMA2D:       DMA2D时钟（STM32F429xx/439xx器件）
  *            @arg RCC_AHB1Periph_ETH_MAC:     以太网MAC时钟
  *            @arg RCC_AHB1Periph_ETH_MAC_Tx:  以太网发送时钟
  *            @arg RCC_AHB1Periph_ETH_MAC_Rx:  以太网接收时钟
  *            @arg RCC_AHB1Periph_ETH_MAC_PTP: 以太网PTP时钟
  *            @arg RCC_AHB1Periph_OTG_HS:      USB OTG HS时钟
  *            @arg RCC_AHB1Periph_OTG_HS_ULPI: USB OTG HS ULPI时钟
