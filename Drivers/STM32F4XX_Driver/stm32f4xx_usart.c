...
/**
  ...
  * @param  USARTx：其中x可以是1、2、3或6，用于选择USART外设。
  * @param  USART_ClockInitStruct：指向USART_ClockInitTypeDef结构体的指针，该结构体包含指定USART外设的配置信息。
  * @note   智能卡模式和同步模式不适用于UART4和UART5。
  * @retval 无
  */
...
/**
  ...
  *         UART外设。
  * @param  USART_Prescaler：指定预分频器时钟。
  * @note   该函数用于UART4和UART5的红外数据协会（IrDA）模式。
  * @retval 无
  */
...
/**
  ...
  * @note   为了获得正确的波特率分频器值，必须在调用USART_Init()函数之前调用此函数。
  * @param  USARTx：其中x可以是1、2、3、4、5、6、7或8，用于选择USART或UART外设。
  * @param  NewState：USART 8倍过采样模式的新状态。
...
/**
  ...
  */
/**
  * @brief  检查指定的USART标志位是否已设置。
  * @param  USARTx：其中x可以是1、2、3、4、5、6、7或8，用于选择USART或
  *         UART外设。
  * @param  USART_FLAG：指定要检查的标志位。
  *          此参数可以是以下值之一：
  *            @arg USART_FLAG_CTS：CTS变化标志（UART4和UART5不适用）
