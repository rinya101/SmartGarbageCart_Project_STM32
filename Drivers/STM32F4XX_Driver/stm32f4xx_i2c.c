...
/**
  ...
  */
/**
  * @brief  将I2Cx外设寄存器恢复为默认复位值。
  * @param  I2Cx：其中x可以是1、2或3，用于选择I2C外设。
  * @retval 无
  */
...
/**
  ...
  *           
  * @note   要使I2C工作在400 KHz（快速模式），PCLK1频率（I2C外设输入时钟）必须是10 MHz的倍数。
  *           
  * @param  I2Cx：x可以是1、2或3，用于选择I2C外设。
  * @param  I2C_InitStruct：指向I2C_InitTypeDef结构体的指针，该结构体包含指定I2C外设的配置信息。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  为每个I2C_InitStruct成员填充其默认值。
  * @param  I2C_InitStruct：指向将要被初始化的I2C_InitTypeDef结构体的指针。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  启用或禁用I2C外设的模拟滤波器。
  * 
  * @note   此函数仅可用于STM32F42xxx/STM3243xxx、STM32F401xx、STM32F410xx和STM32F411xE器件。
  *        
  * @param  I2Cx：x可以是1、2或3，用于选择I2C外设。
  * @param  NewState：模拟滤波器的新状态。
  *          此参数可以是：ENABLE（启用）或DISABLE（禁用）。
  * @note   此函数应在初始化和启用I2C外设之前调用。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  配置I2C外设的数字噪声滤波器。
  * 
  * @note   此函数仅可用于STM32F42xxx/STM3243xxx、STM32F401xx、STM32F410xx和STM32F411xE器件。
  *       
  * @param  I2Cx：x可以为1、2或3，用于选择I2C外设。
  * @param  I2C_DigitalFilter：数字噪声滤波器的系数。
  *          该参数可以是0x00到0x0F之间的一个数值。
  * @note   此函数应在初始化和使能I2C外设之前调用。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  启用或禁用指定的I2C应答功能。
  * @param  I2Cx：其中x可以是1、2或3，用于选择I2C外设。
  * @param  NewState：I2C应答的新状态。
  *          此参数可以是：ENABLE（启用）或DISABLE（禁用）。
  * @retval 无。
  */
...
/**
  ...
  */
/**
  * @brief  启用或禁用指定的I2C双地址模式。
  * @param  I2Cx：x可以是1、2或3，用于选择I2C外设。
  * @param  NewState：I2C双地址模式的新状态。
  *          此参数可以是：ENABLE（启用）或DISABLE（禁用）。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  启用或禁用指定的I2C广播呼叫功能。
  * @param  I2Cx：x可以是1、2或3，用于选择I2C外设。
  * @param  NewState：I2C广播呼叫的新状态。
  *          此参数可以是：ENABLE（启用）或DISABLE（禁用）。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  启用或禁用指定的I2C软件复位。
  * @note   当启用软件复位时，I2C的IO口会被释放（这在从总线错误中恢复时可能有用）。
  * @param  I2Cx：其中x可以是1、2或3，用于选择I2C外设。
  * @param  NewState：I2C软件复位的新状态。
  *          该参数可以是：ENABLE（启用）或DISABLE（禁用）。
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  启用或禁用指定的I2C时钟拉伸。
  * @param  I2Cx：x可以为1、2或3，用于选择I2C外设。
  * @param  NewState：I2Cx时钟拉伸的新状态。
  *          此参数可以是：ENABLE（启用）或DISABLE（禁用）。
  * @retval 无
  */
...
/**
  ...
  *          此参数可以是以下值之一：
  *            @arg I2C_DutyCycle_2：I2C快速模式下Tlow/Thigh = 2
  *            @arg I2C_DutyCycle_16_9：I2C快速模式下Tlow/Thigh = 16/9
  * @retval 无
  */
...
/**
  ...
  */
/**
  * @brief  检查指定的I2C标志位是否已设置。
  * @param  I2Cx：x可以为1、2或3，用于选择I2C外设。
  * @param  I2C_FLAG：指定要检查的标志位。
  *          该参数可以是以下值之一：
  *            @arg I2C_FLAG_DUALF：双重标志（从机模式）
  *            @arg I2C_FLAG_SMBHOST：SMBus主机头部（从机模式）
  *            @arg I2C_FLAG_SMBDEFAULT：SMBus默认头部（从机模式）
  *            @arg I2C_FLAG_GENCALL：通用呼叫头部标志（从机模式）
  *            @arg I2C_FLAG_TRA：发送器/接收器标志
  *            @arg I2C_FLAG_BUSY：总线忙标志
  *            @arg I2C_FLAG_MSL：主机/从机标志
  *            @arg I2C_FLAG_SMBALERT：SMBus警报标志
  *            @arg I2C_FLAG_TIMEOUT：超时或Tlow错误标志
  *            @arg I2C_FLAG_PECERR：接收中的PEC错误标志
  *            @arg I2C_FLAG_OVR：溢出/下溢标志（从机模式）
  *            @arg I2C_FLAG_AF：应答失败标志
  *            @arg I2C_FLAG_ARLO：仲裁丢失标志（主机模式）
  *            @arg I2C_FLAG_BERR：总线错误标志
  *            @arg I2C_FLAG_TXE：数据寄存器空标志（发送器）
  *            @arg I2C_FLAG_RXNE：数据寄存器非空（接收器）标志
  *            @arg I2C_FLAG_STOPF：停止检测标志（从机模式）
  *            @arg I2C_FLAG_ADD10：10位头部已发送标志（主机模式）
  *            @arg I2C_FLAG_BTF：字节传输完成标志
  *            @arg I2C_FLAG_ADDR：地址已发送标志（主机模式）“ADSL”
  *                                地址匹配标志（从机模式）“ENDAD”
  *            @arg I2C_FLAG_SB：起始位标志（主机模式）
  * @retval I2C_FLAG的新状态（SET或RESET）。
  */
...
/**
  ...
  *            @arg I2C_FLAG_TIMEOUT：超时或Tlow错误标志
  *            @arg I2C_FLAG_PECERR：接收中的PEC错误标志
  *            @arg I2C_FLAG_OVR：溢出/欠载标志（从机模式）
  *            @arg I2C_FLAG_AF：应答失败标志
  *            @arg I2C_FLAG_ARLO：仲裁丢失标志（主机模式）
  *            @arg I2C_FLAG_BERR：总线错误标志
  *   
  * @note   STOPF（停止检测）通过软件序列清除：对I2C_SR1寄存器执行读操作
  *          （I2C_GetFlagStatus()），然后对I2C_CR1寄存器执行写操作
  *          （I2C_Cmd()以重新启用I2C外设）。
  * @note   ADD10（10位报头已发送）通过软件序列清除：对I2C_SR1执行读操作
  *          （I2C_GetFlagStatus()），然后在DR寄存器中写入地址的第二个字节。
  * @note   BTF（字节传输完成）通过软件序列清除：对I2C_SR1寄存器执行读操作
  *          （I2C_GetFlagStatus()），然后对I2C_DR寄存器执行读/写操作
  *          （I2C_SendData()）。
  * @note   ADDR（地址已发送）通过软件序列清除：对I2C_SR1寄存器执行读操作
  *          （I2C_GetFlagStatus()），然后对I2C_SR2寄存器执行读操作
  *          （(void)(I2Cx->SR2)）。
  * @note   SB（起始位）通过软件序列清除：对I2C_SR1寄存器执行读操作
  *          （I2C_GetFlagStatus()），然后对I2C_DR寄存器执行写操作
  *          （I2C_SendData()）。
  *  
  * @retval 无
  */
